...
public class CommonServletHandler extends RenderPageServletHandler {
        ...
        if (files != null) {
            ...
            for (String path : paths) {
                ...
                try {
                    ...
                    for (Deserializer<?> des : deserializers) {
                        if (des.support(element)) {
                            ruleSet = (RuleSet) des.deserialize(element);
                            break;
                        }
                    }
                    inputStream.close();
     ...
     * @return
     */
    private String parseCriterion(Criterion criterion) {
        if (criterion instanceof Criteria) {
            ...
            //操作符
            String op = Op.parse(criteria.getOp());
            //值
            SimpleValue simpleValue = (SimpleValue) criteria.getValue();
            String value = simpleValue.getContent();
            ...
            if (criterion instanceof Or) {
                ...
                for (Criterion c : criterions) {
                    expressionList.add(parseCriterion(c));
                }
                return "(" + String.join(" or ", expressionList) + ")";
            }
